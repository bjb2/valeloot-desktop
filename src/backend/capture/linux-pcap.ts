...
export class LinuxPcapRuntime {
  ...
  async status(): Promise<CaptureBackendStatus> {
    ...
    if (process.platform !== "linux") return { availability: "error", detail: "libpcap capture is supported only on Linux" };
    ...
    try {
      ...
      diagnostics.error("Linux capture backend probe failed", { error: formatError(error), missing });
      return { availability: missing ? "missing" : "error", detail: missing ? "libpcap is not installed" : detail };
    }
  }
  ...
  private openDirect(device: CaptureDeviceRecord, filter: string): CaptureSession {
    ...
    try {
      ...
      if (activated === PCAP_ERROR_PERM_DENIED) {
        throw new Error(`libpcap denied access to ${device.description}; install/configure dumpcap or grant the collector CAP_NET_RAW and CAP_NET_ADMIN: ${pcapError(api, handle)}`);
      }
      ...
      diagnostics.info("Direct libpcap session activated", { device, filter, dataLink });
      ...
      while (current) {
        ...
        if (namePointer) {
          ...
          devices.push({
            ...
            loopback: (flags & 1) !== 0 || name.toLocaleLowerCase().includes("loopback"),
          });
        }
  ...
  private load(): PcapLibrary {
    ...
    for (const candidate of candidates) {
      try {
        ...
        diagnostics.info("Loaded libpcap shared library", { library: candidate });
        return this.api;
      } catch (error) {
        diagnostics.debug("Could not load libpcap candidate", { library: candidate, error: formatError(error) });
        lastError = error;
      }
    }
...
class DumpcapSession implements CaptureSession {
  ...
  static async open(executable: string, device: CaptureDeviceRecord, filter: string): Promise<DumpcapSession> {
    ...
    await new Promise<void>((resolve, reject) => {
      let started = false;
      const timeout = setTimeout(() => reject(new Error("dumpcap did not start within 10 seconds")), 10_000);
    ...
    diagnostics.info("dumpcap helper started", { pid: child.pid, dataLink: decoder.dataLink });
    child.once("exit", (code) => {
      diagnostics.info("dumpcap helper exited", { pid: child.pid, code, closed: session.closed, stderr: stderr.trim() });
  ...
  close(): void {
    if (this.closed) return;
    this.closed = true;
    diagnostics.debug("Closing dumpcap session", { pid: this.child.pid, queuedPackets: this.packets.length });
    this.child.kill();
...
export class PcapStreamDecoder {
  ...
  private readGlobalHeader(): boolean {
    ...
    this.dataLink = this.littleEndian ? this.buffer.readUInt32LE(20) : this.buffer.readUInt32BE(20);
